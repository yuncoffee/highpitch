...
extension ProjectSpeakingRateChart {
    ...
    var graph: some View {
        ...
        if let practices = practices, !practices.isEmpty {
            ...
            if let rangeFirst = range.first,
               let rangeLast = range.last,
               let rawFirst = rawRange.first,
               let rawLast = rawRange.last {
                Chart {
                    /// 선 그래프
                    ForEach(Array(practices.enumerated()), id: \.1.id) { index, practice in
                        ...
                        if practice.summary.maxSpm == rawLast {
                            PointMark(
                                x: .value("연습회차", index + 1),
                ...
                .chartYAxis {
                    ...
                    AxisMarks(position: .leading, values: Array(stride(
                        from: rangeFirst,
                        through: rangeLast,
                        by: (rangeLast - rangeFirst) / 4
                    ))) { value in
                        AxisValueLabel(centered: false) {
                            if value.index % 2 == 0 {
                                let axisValue = Double(value.index)
                                    * (rangeLast - rangeFirst) / 4
                                    + rangeFirst
